...
<link rel="stylesheet" href="<?php echo (ADMIN_PUBLIC); ?>/css/weaponAll.css?<?php echo time();?>">
<script src="<?php echo (ADMIN_PUBLIC); ?>/js/weaponAll.js?<?php echo time();?>"></script>
...
<div id="right">
	...
	<?php if(is_array($weapons)): $i = 0; $__LIST__ = $weapons;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$weapon): $mod = ($i % 2 );++$i;?><ul>
			<li><?php echo ($i); ?></li>
			<li><?php echo ($weapon["id"]); ?></li>	
			<li><?php echo ($weapon["name"]); ?></li>	
			<li><?php echo ($weapon["default_attack"]); ?></li>	
			<li><?php echo ($weapon["default_defense"]); ?></li>
			<?php if($weapon["isHaving"] == 1): ?><li><a href="javascript:;">删除</a></li>
				<li>已拥有</li>
			<?php else: ?>
	...
	﻿<div id="page">
	<?php $__FOR_START_950299382__=0;$__FOR_END_950299382__=$pageNum;for($i=$__FOR_START_950299382__;$i < $__FOR_END_950299382__;$i+=1){ if($i == $page): ?><a href=<?php echo U("", array("page" => $i));?> class="page_active"><?php echo ($i+1); ?></a>
		<?php else: ?>
