...
<link rel="stylesheet" href="<?php echo (ADMIN_PUBLIC); ?>/css/dungeonIndex.css?<?php echo time();?>">
<script src="<?php echo (ADMIN_PUBLIC); ?>/js/dungeonIndex.js?<?php echo time();?>"></script>

<div id="dungeon">
	<form method="post" action="/resource/index.php?m=Admin&amp;c=Dungeon&amp;a=index">
		<?php if(is_array($normal_dungeons)): $i = 0; $__LIST__ = $normal_dungeons;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$normal_dungeon): $mod = ($i % 2 );++$i;?><div>
				<?php if(is_array($normal_dungeon)): $i = 0; $__LIST__ = $normal_dungeon;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dungeon): $mod = ($i % 2 );++$i;?><div>
						<label for="<?php echo ($dungeon["id"]); ?>"><?php echo ($dungeon["name"]); ?></label>
						<?php if($dungeon["is_having"] == 1): ?><input type="checkbox" id="<?php echo ($dungeon["id"]); ?>" name="dungeon[]" value="<?php echo ($dungeon["id"]); ?>" checked="checked">
						<?php else: ?>
							
							<input type="checkbox" id="<?php echo ($dungeon["id"]); ?>" name="dungeon[]" value="<?php echo ($dungeon["id"]); ?>"><?php endif; ?>
					</div><?php endforeach; endif; else: echo "" ;endif; ?>
			</div><?php endforeach; endif; else: echo "" ;endif; ?>
		
		<div>
			<label>已选择 <span id="dungeon_num"><?php echo ($dungeon_num); ?></span> 关</label>
			<input type="checkbox" id="elite" name="elite" value="elite" checked="checked">
			<label for="elite">添加相应的精英副本</label>
			<input type="submit" value="提交">
		</div>
	</form>
	<?php if($is_post == 1): ?><span>已成功添加 <?php echo ($dungeon_num); ?> 条记录</span><?php endif; ?>
</div>
