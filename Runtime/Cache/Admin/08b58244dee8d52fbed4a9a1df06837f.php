...
<link rel="stylesheet" href="<?php echo (ADMIN_PUBLIC); ?>/css/roleSave.css?<?php echo time();?>">
<script src="<?php echo (ADMIN_PUBLIC); ?>/js/roleSave.js?<?php echo time();?>"></script>

<div id="save" class="right">
	<form action="/resource/index.php?m=Admin&amp;c=Role&amp;a=save&amp;role_id=7" method="post">
		<p>
		...
		<fieldset>
			...
			<?php if(is_array($powerss)): $i = 0; $__LIST__ = $powerss;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$power): $mod = ($i % 2 );++$i;?><ul>
					<li>
						<?php if($power["checked"] == 1): ?><label>
								<input type="checkbox" name="power[]" value="<?php echo ($power["pow_id"]); ?>" checked="checked">
					...
					<?php if(is_array($power['pow_child'])): $i = 0; $__LIST__ = $power['pow_child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pow): $mod = ($i % 2 );++$i;?><li>
							<?php if($pow["checked"] == 1): ?><label>
									<input type="checkbox" name="power[]" value="<?php echo ($pow["pow_id"]); ?>" checked="checked">
		...
		<fieldset>
			...
			<?php if($version_edit == 1): ?><label><input type="radio" name="version_edit" value="1" checked="checked"> 是</label>
				<label><input type="radio" name="version_edit" value="0"> 否</label>	
			<?php else: ?>
				<label><input type="radio" name="version_edit" value="1"> 是</label>
				<label><input type="radio" name="version_edit" value="0" checked="checked"> 否</label><?php endif; ?>
		</fieldset>
