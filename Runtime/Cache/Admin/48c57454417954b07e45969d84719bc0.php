...
<link rel="stylesheet" href="<?php echo (ADMIN_PUBLIC); ?>/css/mailGroup.css?<?php echo time();?>">
<script src="<?php echo (ADMIN_PUBLIC); ?>/js/mailGroup.js?<?php echo time();?>"></script>
<script src="/resource/Public/datePicker/WdatePicker.js?<?php echo time();?>"></script>

<div id="group" class="right">
	<form action="/resource/index.php?m=Admin&amp;c=Mail&amp;a=group" method="post">
		<?php if($error == 1): ?><span>请选择渠道</span>
		<?php elseif($error == 2): ?>
			<span>请选择分区</span>
		<?php elseif($error == 3): ?>
			<span>标题不能为空</span>
		...
		<p>
			...
			<select id="os" name="os">
				...
				<option value="3">仅 Android</option>	
				<option value="2">仅 ios</option>	
			</select>
		</p>
		<p>
		...
		<p>
			<label for="start">开　始</label>
			<input id="start" name="start" class="Wdate">
			<label for="end">结　束</label>
			<input id="end" name="end" class="Wdate">
		</p>
		<div>
			<div>奖　励</div>
			<ul><li></li></ul>
	...
	<div id="result">
		<?php if($result == 1): ?><p class="success">发送成功：<?php echo ($platform); ?> -- <?php echo ($server); ?> 区 -- <?php echo ($os); ?></p>
		<?php elseif($result == 2): ?>
			<p class="error">发送失败：<?php echo ($platform); ?> -- <?php echo ($server); ?> 区 -- <?php echo ($os); ?></p><?php endif; ?>	
	</div>
</div>
