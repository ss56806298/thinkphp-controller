...
<link rel="stylesheet" href="<?php echo (ADMIN_PUBLIC); ?>/css/helmetAll.css?<?php echo time();?>">
<script src="<?php echo (ADMIN_PUBLIC); ?>/js/helmetAll.js?<?php echo time();?>"></script>
...
<div id="right">
	...
	<?php if(is_array($helmets)): $i = 0; $__LIST__ = $helmets;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$helmet): $mod = ($i % 2 );++$i;?><ul>
			<li><?php echo ($i); ?></li>
			<li><?php echo ($helmet["id"]); ?></li>	
			<li><?php echo ($helmet["name"]); ?></li>	
			<li><?php echo ($helmet["default_attack"]); ?></li>	
			<li><?php echo ($helmet["default_defense"]); ?></li>
			<?php if($helmet["isHaving"] == 1): ?><li><a href="javascript:;">删除</a></li>
				<li>已拥有</li>
			<?php else: ?>
	...
	﻿<div id="page">
	<?php $__FOR_START_1477907587__=0;$__FOR_END_1477907587__=$pageNum;for($i=$__FOR_START_1477907587__;$i < $__FOR_END_1477907587__;$i+=1){ if($i == $page): ?><a href=<?php echo U("", array("page" => $i));?> class="page_active"><?php echo ($i+1); ?></a>
		<?php else: ?>
