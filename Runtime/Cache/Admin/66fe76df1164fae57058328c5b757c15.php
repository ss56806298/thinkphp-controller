...
<link rel="stylesheet" href="<?php echo (ADMIN_PUBLIC); ?>/css/adminIndex.css?<?php echo time();?>">
<script src="<?php echo (ADMIN_PUBLIC); ?>/js/adminIndex.js?<?php echo time();?>"></script>

<div id="admin" class="right">
	<div>
		<a href='<?php echo U("add");?>' class="btn btn-success">新 增</a>	
		<a href="javascript:;" class="btn btn-info">启 用</a>	
		<a href="javascript:;" class="btn btn-warning">锁 定</a>	
		<a href="javascript:;" class="btn btn-danger">删 除</a>
		<a href="javascript:;" class="btn btn-primary">移 动</a>
		<select>
			<option>请选择</option>
			<?php if(is_array($roles)): foreach($roles as $key=>$vo): ?><option value="<?php echo ($vo["role_id"]); ?>"><?php echo ($vo["role_name"]); ?></option><?php endforeach; endif; ?>
		</select>
	</div>
	<ul>
	...
	<?php if(is_array($managers)): $i = 0; $__LIST__ = $managers;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$manager): $mod = ($i % 2 );++$i;?><ul>
			<?php if($manager["mg_islock"] == 1): ?><li><input type="checkbox"></li>
				<li><span class="error"><?php echo ($manager["mg_id"]); ?></span></li>	
				<li><span class="error"><?php echo ($manager["mg_name"]); ?></span></li>	
				<li><span class="error"><?php echo ($manager["mg_true_name"]); ?></span></li>
				<li><span class="error">锁定</span></li>
				<li><span class="error"><?php echo ($manager["role_name"]); ?></span></li>	
				<li><a href='<?php echo U("save", array("mg_id" => $manager["mg_id"]));?>' class="error">编辑</a></li>
			<?php else: ?>
				<li><input type="checkbox"></li>
				<li><?php echo ($manager["mg_id"]); ?></li>	
				<li><?php echo ($manager["mg_name"]); ?></li>	
				<li><?php echo ($manager["mg_true_name"]); ?></li>
				<li>正常</li>
				<li><?php echo ($manager["role_name"]); ?></li>	
				<li><a href='<?php echo U("save", array("mg_id" => $manager["mg_id"]));?>'>编辑</a></li><?php endif; ?>
