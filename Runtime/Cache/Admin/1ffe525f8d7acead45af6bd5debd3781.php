...
<link rel="stylesheet" href="<?php echo (ADMIN_PUBLIC); ?>/css/roleIndex.css?<?php echo time();?>">
<script src="<?php echo (ADMIN_PUBLIC); ?>/js/roleIndex.js?<?php echo time();?>"></script>

<div id="role" class="right">
	<div>
	...
	<?php if(is_array($roles)): $i = 0; $__LIST__ = $roles;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$role): $mod = ($i % 2 );++$i; if($role["state"] == 0): ?><ul>
				<li><input type="checkbox"></li>
				<li><?php echo ($role["role_id"]); ?></li>	
				<li><?php echo ($role["role_name"]); ?></li>
				<li>正常</li>
				<li><a href='<?php echo U("Admin/index", array("role_id" => $role["role_id"]));?>'>角色成员</a></li>	
				<li><a href='<?php echo U("save", array("role_id" => $role["role_id"]));?>'>编辑</a></li>	
			</ul>
		<?php elseif($role["state"] == 1): ?>
			<ul>
				<li><input type="checkbox"></li>
				<li class="state1"><?php echo ($role["role_id"]); ?></li>	
				<li class="state1"><?php echo ($role["role_name"]); ?></li>
				<li class="state1">锁定</li>
				<li><a href='<?php echo U("Admin/index", array("role_id" => $role["role_id"]));?>' class="state1">角色成员</a></li>	
				<li><a href='<?php echo U("save", array("role_id" => $role["role_id"]));?>' class="state1">编辑</a></li>	
			</ul>
		<?php else: ?>	
			<ul>
				<li><input type="checkbox"></li>
				<li class="state2"><?php echo ($role["role_id"]); ?></li>	
				<li class="state2"><?php echo ($role["role_name"]); ?></li>
				<li class="state2">部分锁定</li>
				<li><a href='<?php echo U("Admin/index", array("role_id" => $role["role_id"]));?>' class="state2">角色成员</a></li>	
				<li><a href='<?php echo U("save", array("role_id" => $role["role_id"]));?>' class="state2">编辑</a></li>	
