...
<link rel="stylesheet" href="<?php echo (ADMIN_PUBLIC); ?>/css/adminSave.css?<?php echo time();?>">
<script src="<?php echo (ADMIN_PUBLIC); ?>/js/adminSave.js?<?php echo time();?>"></script>

<div id="save" class="right">
	<form action="/resource/index.php?m=Admin&amp;c=Admin&amp;a=save&amp;mg_id=2" method="post">
		<p>
			<input type="hidden" name="mg_id" value="<?php echo ($manager["mg_id"]); ?>">	
		</p>
		<p>
		...
		<p>
			...
			<input type="text" id="mg_true_name" name="mg_true_name" value="<?php echo ($manager["mg_true_name"]); ?>">
			<span></span>
		</p>
		<p>
			<label for="mg_name">用 户 名</label>
			<input type="text" id="mg_name" name="mg_name" value="<?php echo ($manager["mg_name"]); ?>">
			<span></span>
		</p>
		<p>
		...
		<p>
			<label>状　　态</label>
			<?php if($manager["mg_islock"] == 1): ?><label>
					<input type="radio" name="state" value="0"/>正常	
				</label>
				<label>
					<input type="radio" name="state" value="1" checked="checked"/>锁定	
				</label>
			<?php else: ?>
				<label>
				...
				<label>
					<input type="radio" name="state" value="1"/>锁定	
				</label><?php endif; ?>	
		</p>
		<p>
			<label for="role">选择角色</label>
			<select id="role" name="role">
				<?php if(is_array($roles)): $i = 0; $__LIST__ = $roles;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$role): $mod = ($i % 2 );++$i; if($manager["role_id"] == $role['role_id']): ?><option value="<?php echo ($role["role_id"]); ?>" selected="selected"><?php echo ($role["role_name"]); ?></option>
					<?php else: ?>
